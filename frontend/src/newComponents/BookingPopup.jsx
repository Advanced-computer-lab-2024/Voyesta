...
const BookingPopup = ({ item, itemType, onClose, onBook }) => {
  ...
  const availableDates = itemType === 'itinerary' ? item.availableDates : [item.date];

  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-4 rounded shadow-lg">
        <h2 className="text-xl mb-4">Book {itemType === 'itinerary' ? 'Itinerary' : 'Activity'}: {item.name}</h2>
